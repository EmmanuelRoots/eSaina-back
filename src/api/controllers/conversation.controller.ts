...
export class ConversationController extends Controller {
  ...
  public async getAllConversationByUser(
    ...
    return conversationSa.getAllConversationByUser(
      (req as any).user.id,
      Number(page),
      Number(limit)
    )
  ...
  public async createConversation(@Request() req: ExpressRequest) {
    return conversationSa.createConversation(
      (req as any).user.id,
  ...
  @Middlewares([authMiddleware])
  public async getAllMessagesByConversation(
    @Query() conversationId: string,
    @Query() page = 1,
    @Query() limit = 20
  ) {
    return conversationSa.getAllMessagesByConversation(
      conversationId,
      page,
      limit
    )
